const STORAGE_KEY = "halfspace-bachelorette-v1";
const MAX_BETS = 3;
...
const SEASON_BET_CATEGORIES = [
  ...
  { key: "episode-first-kiss",     label: "Week of first kiss",                desc: "Pick the week number (1\u2013N). Closest wins.",                                      points: 12, inputType: "week" },
...
/** Bachelorette season 4 (2026) — photos via femina.dk imgix CDN */
const CAST_BA4_2026 = [
  {
    name: "Adam",
    age: 23,
  ...
  },
  {
    name: "Anders",
    age: 27,
    occupation: "MSc Management Accounting and Control",
...
function loadState() {
  try {
    ...
    if (!Array.isArray(result.players) || result.players.length === 0) result.players = [...DEFAULT_PLAYERS];
...
if (!state.players.includes(currentPlayer)) currentPlayer = "";
...
// Rolling auto-backup. Every successful save schedules a debounced
// snapshot of the full state to backups/<timestamp>. We keep the last
// BACKUP_KEEP entries so a few hours of activity stay recoverable even
// if Firebase /state gets clobbered.
...
function getDayOfWeekName(dateStr) {
  ...
  return ENGLISH_DAYS_LONG[dow] || ENGLISH_DAYS[dow] || "";
}

function formatDate(dateStr) {
  if (!dateStr) return "";
  const [, m, d] = dateStr.split("-").map(Number);
  return `${d} ${ENGLISH_MONTHS[m - 1]}`;
}

function formatDateRange(startStr, endStr) {
  if (!startStr || !endStr) return formatDate(startStr || endStr);
  const [, sm, sd] = startStr.split("-").map(Number);
  const [, em, ed] = endStr.split("-").map(Number);
  if (sm === em) return `${sd}\u2013${ed} ${ENGLISH_MONTHS[sm - 1]}`;
  return `${sd} ${ENGLISH_MONTHS[sm - 1]} \u2013 ${ed} ${ENGLISH_MONTHS[em - 1]}`;
}
...
function computeWeeklyRecap(weekId) {
  ...
  const weeklyPoints = state.players.map(() => 0);
  const perEpisode = [];
  ...
  for (const ep of eps) {
    ...
    const dayName = getDayOfWeekName(ep.airDate);
      ...
      for (const evId of picks) {
        ...
        if (occurred.has(evId)) {
          ...
          if (!biggestPayout || odds > biggestPayout.odds) {
            biggestPayout = { player: state.players[p], text: evKey, odds, day: dayName };
          }
          if (!epMoment || odds > epMoment.odds) {
            epMoment = { type: "win", player: state.players[p], text: evKey, odds };
          }
        } else {
          if (!biggestWhiff || odds > biggestWhiff.odds) {
            biggestWhiff = { player: state.players[p], text: evKey, odds, day: dayName };
          }
        }
      }
    }
    ...
    perEpisode.push({
      ...
      airDate: ep.airDate,
      dayOfWeek: dayName,
      winner: state.players[epWinnerIdx],
  ...
  const streakBonus = state.players.map((_, i) => {
    let bonus = 0;
    for (const ep of eps) {
  ...
  for (let p = 0; p < state.players.length; p++) weeklyPoints[p] += streakBonus[p];

  const scoreboard = state.players.map((_, i) => {
    let bestEpDay = "";
    let bestEpPts = -1;
  ...
  if (ceremonyEp) {
    ...
    if (elimSet.size === 0) {
      ...
      if (uniqueCallers.length === 0) {
        const names = [...elimSet].join(" and ");
        eliminationCallText = `A week of surprises \u2014 no one saw it coming`;
      } else if (uniqueCallers.length === 1) {
        eliminationCallText = `\uD83C\uDFAF Only ${state.players[uniqueCallers[0]]} nailed it`;
      }
    }
  }
  ...
  const hotStreak = streaks.reduce((best, s, i) => {
    ...
    return cur > (best?.count || 0) ? { player: state.players[i], count: cur } : best;
  }, null);
  ...
  if (prevWeekIds.length) {
    const prevTotals = state.players.map(() => 0);
    for (const ep of state.episodes) {
      const wid = getWeekId(ep.airDate);
    ...
    prevRanks = state.players.map((_, i) => prevSorted.findIndex((s) => s.i === i) + 1);
  }
...
function renderPlayersManager() {
  ...
  const toggle = document.querySelector("#players-manager");
  if (toggle) {
    toggle.style.cssText = "margin:0.75rem 0;";
    const summary = toggle.querySelector("summary");
    if (summary) summary.style.cssText = "cursor:pointer;font-size:0.8rem;opacity:0.7;user-select:none;";
    const body = toggle.querySelector(".players-manager__body");
    if (body) body.style.cssText = "margin-top:0.5rem;";
    const addRow = toggle.querySelector(".players-manager__add");
    if (addRow) addRow.style.cssText = "display:flex;gap:0.4rem;align-items:center;";
  }
}
...
function addPlayerFromInput() {
  ...
  if (state.players.includes(name)) {
    showToast(`${name} is already a player.`);
    return;
  }
  state.players.push(name);
  for (const ep of state.episodes) {
    ensurePlayerSlots(ep.id);
  }
  input.value = "";
  saveState();
...
function removePlayer(index) {
  ...
  for (const epId of Object.keys(state.bets)) {
    ...
    if (typeof epBets === "object") {
      const rebuilt = {};
      let src = 0;
      for (let dst = 0; src < Object.keys(epBets).length + 1; src++) {
        if (src === index) continue;
        if (Array.isArray(epBets[src])) rebuilt[dst] = epBets[src];
        dst++;
      }
      state.bets[epId] = rebuilt;
    }
  }
  for (const epId of Object.keys(state.eliminationBets || {})) {
    const epElim = state.eliminationBets[epId];
    if (typeof epElim === "object") {
      const rebuilt = {};
      let src = 0;
      for (let dst = 0; src < Object.keys(epElim).length + 1; src++) {
        if (src === index) continue;
        if (epElim[src] !== undefined) rebuilt[dst] = epElim[src];
        dst++;
      }
      state.eliminationBets[epId] = rebuilt;
    }
  }
  for (const epId of Object.keys(state.nuttet || {})) {
    const epNuttet = state.nuttet[epId];
    if (typeof epNuttet === "object") {
      const rebuilt = {};
      let src = 0;
      for (let dst = 0; src < Object.keys(epNuttet).length + 1; src++) {
        if (src === index) continue;
        if (epNuttet[src] !== undefined) rebuilt[dst] = epNuttet[src];
        dst++;
      }
      state.nuttet[epId] = rebuilt;
    }
  }
  const seasonRebuilt = {};
  for (const [k, v] of Object.entries(state.seasonBets || {})) {
    const ki = parseInt(k, 10);
    if (ki === index) continue;
    seasonRebuilt[ki < index ? ki : ki - 1] = v;
  }
...
function ensurePlayerSlots(epId) {
  ensureEpisodeMaps(epId);
  const bets = state.bets[epId];
  for (let i = 0; i < state.players.length; i++) {
    if (!Array.isArray(bets[i])) bets[i] = [];
  }
}
...
function renderOverview() {
  ...
  allCast().forEach((c) => {
    ...
    editPhoto.textContent = c.photo ? "edit" : "+ photo";
    ...
    if (nuttetCount > 0) {
      ...
      nuttetBadge.textContent = `Cute score: ${nuttetCount}`;
...
function renderResults() {
  ...
  betEvents.forEach((ev) => {
    ...
    for (let p = 0; p < state.players.length; p++) {
      if ((state.bets[ep.id][p] || []).includes(ev.id)) whoPickedNames.push(state.players[p]);
    }
    const span = document.createElement("span");
...
function renderEpisodeScoreSummary() {
  ...
  const streaks = getStreaks();
  const lines = [];
  const parts = state.players.map((name, i) => {
    const bd = streaks[i]?.episodeBreakdown?.find((b) => b.episodeId === ep.id);
    let extra = "";
    if (bd?.bonus > 0) {
...
function computePlayerStats(p) {
  ...
  const weeklyTrend = state.episodes.map((ep, i) => ({
    weekLabel: `Ep ${i + 1}`,
    points: epTotals[i][p],
    groupAvg: Math.round((epTotals[i].reduce((a, b) => a + b, 0) / state.players.length) * 10) / 10,
  }));
  ...
  const comparisons = {};
  const allHitRates = state.players.map((_, i) => {
    let b = 0, h = 0;
    for (const ep of state.episodes) {
      ensureEpisodeMaps(ep.id);
  ...
  const allBiggest = state.players.map((_, i) => {
    let best = 0;
    for (const ep of state.episodes) {
      ensureEpisodeMaps(ep.id);
      const occ = new Set(state.occurred[ep.id] || []);
  ...
  const allElimCorrect = state.players.map((_, i) => {
    let c = 0;
    for (const ep of state.episodes) {
      ensureEpisodeMaps(ep.id);
      ensureEliminatedArray(ep);
  ...
  const notesWritten = Object.keys(state.contestantNotes || {}).filter((k) => {
    ...
    return n?.lastEditedBy === state.players[p] && n?.text;
  }).length;
...
function renderMyStats() {
  ...
  if (statsViewMode === "overview") {
    ...
    h2.id = "my-stats-heading";
    h2.className = "page-hero__title";
    h2.textContent = `${state.players[p]}'s stats`;
    hero.append(h2);
    headerRoot.append(hero);
...
function renderAllPlayersOverview(root) {
  const allStats = state.players.map((_, i) => computePlayerStats(i));
    ...
    <div class="about-numbered">
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>Before each week, each of us picks <strong>3 events</strong> from a bank of way too many bets \u2014 things like \u201Csomeone cries,\u201D \u201Ca group date goes wrong,\u201D or \u201CStig Rossen makes an appearance.\u201D Each event has odds. Rarer events pay more. This is gambling for people who are too anxious to actually gamble, despite working in a company that was built on gambling.</p>
        </div>
      </div>
      <div class="about-numbered__item">
        <span class="about-numbered__num">02</span>
        <div class="about-numbered__body">
          <h3 class="about-numbered__title">Elimination bet</h3>
          <p>Each week comes with an <strong>extra elimination bet</strong>: who\u2019s going home? Correct guesses pay out based on how many contestants are still in the running. Early-season guesses are genuinely hard. Late-season guesses are mostly vibes.</p>
        </div>
      </div>
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>Monday at 23:59, all bets lock \u2014 no late entries, no edits, no mercy. We\u2019re colleagues. We still don\u2019t trust each other.</p>
        </div>
      </div>
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>After the week, the admin ticks off what actually happened and who got eliminated. Points get tallied automatically and the leaderboard updates instantly.</p>
        </div>
      </div>
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>Every week, each player picks which contestant they thought was cutest. It\u2019s worth zero points \u2014 pure vibes, no strategy. A running tally on the overview page shows who the group collectively finds most adorable. It\u2019s a feelings column.</p>
        </div>
      </div>
      <div class="about-numbered__item">
      ...
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>The app tracks everything: hit rates, streaks, total points per week, and detailed player stats. Each week there\u2019s a summary you can copy and share \u2014 leaderboard, standout moments, and who\u2019s climbing or falling.</p>
        </div>
      </div>
      <div class="about-numbered__item">
        ...
        <div class="about-numbered__body">
          ...
          <p>No refreshing, no \u201Cdid you see my bet?\u201D Everyone sees everything in real time, across all devices. Open it on your phone while watching or check the leaderboard on your laptop the next morning \u2014 always up to date.</p>
        </div>
      </div>
    </div>
  </div>
...
<section class="about-split">
  ...
  <div class="about-split__content">
    ...
    <p>Odds aren\u2019t sacred. We tune them between weeks. If \u201Csomeone cries\u201D keeps happening 100% of the time, that is feedback from the universe and we take it seriously.</p>
  </div>
</section>
  ...
  <div class="about-split__content">
    <div class="about-fairplay">
      <div class="about-fairplay__block">
        ...
        <p>Waiting to see what happens in a week before locking in your bets. That\u2019s why betting closes Monday at 23:59. We love each other. We don\u2019t trust each other.</p>
      </div>
      <div class="about-fairplay__block">
...
function renderLeaderboard() {
  ...
  const elimCorrect = state.players.map(() => 0);
  for (const ep of state.episodes) {
    ensureEpisodeMaps(ep.id);
    ensureEliminatedArray(ep);
    const elimSet = new Set(ep.eliminated);
  ...
  order.forEach(({ i, pts }, rank) => {
    ...
    nameEl.textContent = state.players[i];
...
function renderAdminPanel() {
  ...
  if (!isAdmin()) {
    ...
    hint.textContent = "Enter the admin password to access this panel.";
    const btn = document.createElement("button");
    btn.type = "button";
  ...
  state.episodes.forEach((ep, idx) => {
    ...
    info.textContent = (ep.title || `Week ${idx + 1}`) + status;
    left.append(info);
    ...
    if (!ep.closed && !ep.betsLocked) {
      actions.append(adminBtn("Lock bets", "btn--warning-sm", () => {
        if (!confirm(`Lock bets for ${ep.title || "Week " + (idx + 1)}?`)) return;
        ep.betsLocked = true;
    ...
    if (!ep.closed) {
      actions.append(adminBtn("Close", "btn--warning-sm", () => {
        if (!confirm(`Close ${ep.title || "Week " + (idx + 1)}? Scores will be finalized.`)) return;
        ep.closed = true;
        ...
        if (!nextEp) {
          ...
          state.episodes.push({ id: uid(), title: `Week ${state.episodes.length + 1}`, guys: carryGuys, events: [], eliminated: [], betsLocked: false, betsLockedAt: null, betsLockDeadline, elimBetsLocked: false, elimBetsLockDeadline, airDate: nextAirDate });
        }
        saveState();
        renderAdminPanel();
        renderMainTabs();
    ...
    if (ep.closed) {
      actions.append(adminBtn("Reopen", "btn--secondary-sm", () => {
        if (!confirm(`Reopen ${ep.title || "Week " + (idx + 1)}? Be careful.`)) return;
        ep.closed = false;
        ep.betsLocked = false;
        saveState();
        renderAdminPanel();
    ...
    actions.append(adminBtn("Delete", "btn--danger-sm", () => {
      if (!confirm(`Delete ${ep.title || "Week " + (idx + 1)}? This cannot be undone.`)) return;
  ...
  const addEpBtn = adminBtn("+ Add week", "btn--secondary", () => {
    ...
    const newEp = { id: uid(), title: `Week ${n}`, guys: carryGuys, events: [], eliminated: [], betsLocked: false, betsLockedAt: null, betsLockDeadline: null, airDate: null };
...
function renderAllBetsOverview() {
  ...
  for (let epIdx = 0; epIdx < epCount; epIdx++) {
    ...
    for (const { p, picks, elimPick } of playerRows) {
      ...
      playerName.textContent = state.players[p];
...
function renderNuttetSection(ep) {
  ...
  for (let p = 0; p < state.players.length; p++) {
    ...
    nameEl.textContent = state.players[p];
...
function renderPlayerSections() {
  ...
  for (let p = 0; p < state.players.length; p++) {
    ...
    header.textContent = state.players[p];
    card.append(header);
...
function renderEpisodeContent() {
  ...
  if (heroEl) {
    ...
    if (ep) {
      ...
      const tagline = document.createElement("p");
      tagline.className = "page-hero__tagline";
      tagline.textContent = ep.airDate ? formatWeekDateRange(ep.airDate) : "";
      ...
      if (ep.betsLockDeadline && !ep.betsLocked && !closed) {
        ...
        if (diff <= 0) {
          ...
          strip.textContent = "Bets lock " + formatDeadlineTime(dl) + " (\u2248 " + formatCountdown(dl) + ")";
...
function renderDeadlineDisplay(el, ep) {
  ...
  if (ep.betsLocked) {
    ...
    el.textContent = "Locked" + (ep.betsLockedAt ? " \u00B7 " + formatDeadlineTime(ep.betsLockedAt) : "");
  } else if (now >= dl) {
    el.className = "deadline-display deadline-display--warn";
    el.textContent = "Auto-locking now\u2026";
  } else {
    const diff = dl - now;
...
function wireActions() {
  ...
  document.getElementById("add-event")?.addEventListener("click", () => {
    ...
    if (!text) { showToast("Write an event text first."); return; }
...
function generateRecapMarkdown(recap) {
  ...
  if (recap.highlights.biggestPayout || recap.highlights.biggestWhiff) {
    ...
    if (recap.highlights.biggestPayout) {
      ...
      md += `- Win of the week: ${h.player} hit '${h.text}' at ${formatNumber(h.odds)}x (${h.day})\n`;
    }
    if (recap.highlights.biggestWhiff) {
      const h = recap.highlights.biggestWhiff;
      md += `- Flop of the week: ${h.player} bet on '${h.text}' at ${formatNumber(h.odds)}x (${h.day})\n`;
    }
    md += "\n";
  }
  ...
  if (recap.eliminations.length) {
    ...
    for (const e of recap.eliminations) {
      const callerNames = e.calledBy.map((i) => state.players[i]);
      if (callerNames.length) {
        md += `- ${e.contestantName} went home. ${callerNames.join(" and ")} nailed it.\n`;
      } else {
        md += `- ${e.contestantName} went home. No one saw it coming.\n`;
      }
    }
    md += "\n";
  }
  ...
  recap.seasonStandings.forEach((s) => {
    const name = state.players[s.playerIndex];
    md += `- ${name} \u2014 ${formatNumber(s.totalPoints)} pts\n`;
  });
...
function renderWeeklyRecap() {
  ...
  if (!completeWeeks.length) {
    const empty = document.createElement("p");
    empty.className = "panel__hint";
    empty.textContent = "No closed weeks yet. Close a week to see the summary.";
    root.append(empty);
    return;
  }
  ...
  for (let wi = 0; wi < completeWeeks.length; wi++) {
    ...
    sbTitle.textContent = "Weekly standings";
    ...
    recap.scoreboard.forEach((s, idx) => {
      ...
      name.textContent = state.players[s.playerIndex];
      const pts = document.createElement("span");
      pts.className = "recap-sb-row__pts";
      pts.textContent = `${formatNumber(s.weekPoints)} pts`;
      ...
      if (s.bestEpisodeDay && s.bestEpisodePts > 0) {
        ...
        best.textContent = `Best week: ${s.bestEpisodeDay}, ${formatNumber(s.bestEpisodePts)} pts`;
        row.append(best);
      }
    ...
    if (hl.biggestPayout) {
      hlItems.push({
        ...
        text: `${hl.biggestPayout.player} hit '${hl.biggestPayout.text}' at ${formatNumber(hl.biggestPayout.odds)}x (${hl.biggestPayout.day})`,
        accent: "gold",
      });
    }
    if (hl.biggestWhiff) {
      hlItems.push({
        icon: "",
        title: "Biggest flop of the week",
        text: `${hl.biggestWhiff.player} bet on '${hl.biggestWhiff.text}' at ${formatNumber(hl.biggestWhiff.odds)}x (${hl.biggestWhiff.day}). It didn\u2019t happen.`,
        accent: "coral",
      });
    }
    if (hl.hotStreak) {
    ...
    if (hl.mostPlayedEvent) {
      ...
      hlItems.push({
        icon: "",
        title: "Most played event of the week",
    ...
    if (hlItems.length) {
      ...
      hlTitle.textContent = "Highlights of the week";
    ...
    if (recap.eliminations.length || recap.highlights.eliminationCallText === "No eliminations this week") {
      ...
      roseTitle.textContent = "Thursday\u2019s rose ceremony";
      ...
      if (!recap.eliminations.length) {
        ...
        for (const el of recap.eliminations) {
          ...
          if (el.calledBy.length) {
            ...
            callers.textContent = "Called by " + el.calledBy.map((i) => state.players[i]).join(", ");
    ...
    for (const s of recap.seasonStandings) {
      ...
      nameEl.textContent = state.players[s.playerIndex];
      const ptsEl = document.createElement("span");
      ptsEl.className = "recap-stand-row__pts";
      ptsEl.textContent = `${formatNumber(s.totalPoints)} pts`;
      ...
      if (s.distanceToFirst > 0) {
        ...
        dist.textContent = `${formatNumber(s.distanceToFirst)} pts behind`;
        row.append(dist);
      }
...
function checkNewRecapBanner() {
  ...
  if (unseen) {
    ...
    banner.innerHTML = `<span>Week ${seasonWeekNum} recap is ready</span>`;
    const btn = document.createElement("button");
    btn.type = "button";
...
function renderAll() {
  ...
  safe(renderPlayersManager, "renderPlayersManager");
}
...
function autoLockTick() {
  ...
  for (const ep of state.episodes) {
    if (shouldAutoLock(ep, now)) {
      ...
      showToast("Event bets auto-locked for " + episodeTabLabel(idx));
    }
    if (shouldAutoLockElim(ep, now)) {
      ep.elimBetsLocked = true;
      changed = true;
      const idx = state.episodes.indexOf(ep);
      showToast("Elimination bets auto-locked for " + episodeTabLabel(idx));
    }
  }
  if (changed) {
    saveState();
